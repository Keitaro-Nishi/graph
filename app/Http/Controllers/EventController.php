...
class EventController
{
	public function index(Request $request)
	{
		...
		$genrelist= array();
		$genrelists= array();
		$genres= Event::where('citycode', $cityCD)->orderBy('gid1', 'ASC')->orderBy('gid2', 'ASC')->get();
		$genregid1 = Event::select('gid1')->where('citycode', $cityCD)->get();
		$j1values = Event::where('citycode', $cityCD)->where('bunrui', 1)->get();
		foreach ($genres as $genre) {
			...
			if($bunrui == 2){
				$bunruidata= Event::select('meisho')->where('citycode',$cityCD)->where('bunrui',1)->where('gid1',$gid1)->first();
				$shoubunrui = $meisho;
	...
	public function delete()
	{
		...
		foreach ($idsdata as $iddata) {
			...
			$gid1 = $aos[0];
			$gid2 = $aos[1];
			$g2meishodata = Event::select('meisho')->where('citycode',$cityCD)->where('gid1',$gid1)->where('gid2',$gid2)->first();
			$g2meisho = $g2meishodata->meisho;
			if($gid2 == 0){
				$gid2datas = Event::select('gid2')->where('citycode',$cityCD)->where('gid1',$gid1)->get();
				//CVS削除
				//dialog_node
				...
				$watson->callcvsDelete($cityCD,$url);
				Event::where('citycode',$cityCD)->where('gid1',$gid1)->delete();
			}else{
				Event::where('citycode',$cityCD)->where('gid1',$gid1)->where('gid2',$gid2)->delete();
				//CVS削除
	...
	public function update()
	{
		...
		$watson = new Watson;
		if($meisho == null){
			return \Response::json(['status' => 'blunk']);
		}else{
			if($uiKbn == 1){
				Event::where('citycode',$cityCD)->where('gid1',$gid1)->where('gid2',$gid2)->update(['meisho' => $meisho]);
				if($gid2 == 0){
					//大分類
					//CVSデータ修正
					//Intents
				...
				if($bunrui == 1){
					//大分類
					$gid1data= Event::select('gid1')->where('citycode', $cityCD)->orderBy('gid1', 'DESC')->first();
					if(!$gid1data){
						$gid1 =1;
					}else{
